...
Tai lieu nay la checklist nghiem thu theo tung gate cho pipeline trong `docs/02-member-2-reasoning-nlp/`.
...
- [ ] Text field bat buoc khong rong sau trim.
...
- [ ] Output chi la JSON, khong chen text ngoai.
- [ ] Co day du key: `plot_summary`, `moral_lesson`, `quality_flags`, `evidence`.
- [ ] `plot_summary` va `moral_lesson` khong rong sau trim.
- [ ] `evidence.timestamps` ton tai trong context.
- [ ] Retry policy duoc ap dung neu parse fail.
- [ ] Neu xuat `final_summary.json` noi bo, file pass `contracts/v1/final_summary.schema.json`.
...
- [ ] Deliverable I/O chi duoc validate bang schema trong `contracts/v1/`.
- [ ] Khong chen metadata noi bo (nhu `quality_flags`, `evidence`, `confidence`, `role`) vao deliverable neu schema global khong cho phep.
